...
export type NodeId = string & { __name?: 'NodeId' };

/** Unique string identifier among all `Edge`s in a layout. */
export type EdgeId = string & { __name?: 'EdgeId' };
...
export type Node = {
    ...
    ports: Record<
        string,
        {
            ...
            point: Vector;
        } // TODO: Convert this to Map<string, ...>;
    >;
...
export function fromSchema(
    ...
    const nodes: Node[] = nodeSchemas.map(
        ({ id, center, shape, fixed, children, ports, meta }, idx) => {
            ...
            return {
                ...
                ports: ports
                    ? mapValues(ports, ({ location, order, point }) => ({
                          location: location || portLocation,
                          order,
                          point: point
                              ? new Vector(point.x, point.y)
                              : new Vector(
                                    centerpt.x + kPortOffset * rand(),
                                    centerpt.y + kPortOffset * rand(),
                                ),
                      }))
                    : {},
                meta,
    ...
    nodes.forEach(
        (node) =>
            (node.children = node.children.map((id: any) => {
                const childIdx = nodeIdToIdx.get(id);
                if (childIdx === undefined)
                    throw Error(`Invalid child NodeId: ${id}, parent ${node.id}`);
    ...
    function processEndpoint(
        ...
        if (port) {
            if (port in node.ports) {
                ...
                throw Error(
                    `Invalid ${type} port name: ${port}, ${type} ${nodeId}, edge ${edgeId}`,
                );
            }
        } else {
            ...
            port = `_${edgeId}`;
            point = new Vector(
                node.center.x + kPortOffset * rand(),
                node.center.y + kPortOffset * rand(),
            );
...
export function toSchema(
    nodes: Node[],
    edges: Edge[],
): { nodeSchemas: NodeSchema[]; edgeSchemas: EdgeSchema[] } {
    const nodeSchemas = nodes.map((node) => ({
        ...
        ports: Object.fromEntries(
            Object.entries(node.ports).map(([name, { location, order, point }]) => [
                name,
                { location, order, point: { x: point.x, y: point.y } },
            ]),
        ),
        meta: node.meta,
    }));
