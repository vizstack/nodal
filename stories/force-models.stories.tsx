...
storiesOf('force models', module)
    ...
    .add('spring w/ simple nodes', () => {
        ...
        const optimizerType = select(
            'optimizer',
            ['EnergyOptimizer', 'RMSPropOptimizer', 'BasicOptimizer', 'TrustRegionOptimizer'],
            'EnergyOptimizer',
        );
        ...
        switch (optimizerType) {
            case 'BasicOptimizer':
                forceOptimizer = new BasicOptimizer(0.5, 1);
                break;
            case 'RMSPropOptimizer':
                forceOptimizer = new RMSPropOptimizer(2.0, 0.8);
                break;
            case 'TrustRegionOptimizer':
                forceOptimizer = new TrustRegionOptimizer(1, 0.01, 0.9, 10, 0.01);
                break;
            default:
            case 'EnergyOptimizer':
        ...
        const layout = new StagedLayout(
            ...
            { steps },
            {
                ...
                optimizer: forceOptimizer,
                generator: function*(storage) {
                    yield* generateSpringForces(storage, idealLength, shortestPath);
                },
            },
            {
                iterations: 3,
    ...
    .add('spring-majorization w/ simple nodes', () => {
        const steps = number('# timesteps', 100, { range: true, min: 0, max: 200, step: 1 });
        const idealLength = number('ideal length', 15);

        const { nodes, edges } = fromSchema(kGraphSimple.nodes, kGraphSimple.edges);
        // const { nodes, edges } = fromSchema(kGraphFive.nodesEqual, kGraphFive.edgesCyclic);
        const storage = new StructuredStorage(nodes, edges);
        const shortestPath = storage.shortestPaths();
        ...
        const layout = new StagedLayout(
            ...
            { steps },
            {
                ...
                optimizer: forceOptimizer,
                generator: function*(storage) {
                    yield* generateSpringForcesMajorization(storage, idealLength, shortestPath);
                },
            },
            {
                iterations: 3,
    ...
    .add('spring-newton w/ simple nodes', () => {
        const steps = number('# timesteps', 100, { range: true, min: 0, max: 200, step: 1 });
