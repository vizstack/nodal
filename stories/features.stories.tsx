...
storiesOf('features', module)
    .add('simple nodes', () => {
        ...
        const layout = makeLayout(kGraphFive.nodesEqual, kGraphFive.edgesAcyclic, { idealLength });
        return <Graph key={`${Math.random()}`} layout={layout} animated interactive />;
    })
    .add('simple nodes (unequal)', () => {
        const idealLength = number('ideal length', 30);
        const layout = makeLayout(kGraphFive.nodesUnequal, kGraphFive.edgesAcyclic, {
            idealLength,
        });
        return <Graph key={`${Math.random()}`} layout={layout} animated interactive />;
    })
    .add('simple nodes (shapes)', () => {
        const idealLength = number('ideal length', 30);
        const layout = makeLayout(kGraphFive.nodesShapes, kGraphFive.edgesAcyclic, { idealLength });
        return <Graph key={`${Math.random()}`} layout={layout} animated interactive />;
    })
    .add('disconnected w/ non-overlap', () => {
        const idealLength = number('ideal length', 30);
        const margin = number('margin', 0);
        const layout = makeLayout(kGraphFive.nodesUnequal, [], {
            idealLength,
    ...
    .add('compound nodes', () => {
        ...
        const layout = makeLayout(kGraphFive.nodesNested, kGraphFive.edgesAcyclic, {
            idealLength,
            compactness,
        });
        return <Graph key={`${Math.random()}`} layout={layout} animated interactive />;
    ...
    .add('compound nodes (shapes)', () => {
        const idealLength = number('ideal length', 30);
        const compactness = number('group compactness', 0.5);
        const layout = makeLayout(kGraphFive.nodesNestedShapes, kGraphFive.edgesAcyclic, {
            idealLength,
            compactness,
        });
        return <Graph key={`${Math.random()}`} layout={layout} animated interactive />;
    ...
    .add('unidirectional flow', () => {
        ...
        const layout = makeLayout(kGraphFive.nodesEqual, kGraphFive.edgesAcyclic, {
            idealLength,
    ...
    .add('multidirectional flow', () => {
        const idealLength = number('ideal length', 30);
        const flowSeparation = number('flow separation', 50);
        const layout = makeLayout(kGraphFive.nodesUnequal, kGraphFive.edgesTree, {
            idealLength,
            extraConstraints: function*(storage) {
                function constrain(uid: NodeId, vid: NodeId, direction: [number, number]) {
                    const u = storage.node(uid);
                    const v = storage.node(vid);
                    return constrainOffset(u.center, v.center, '>=', flowSeparation, direction, {
                        masses: [u.fixed ? 1e9 : 1, v.fixed ? 1e9 : 1],
                    });
                }
    ...
    .add('named ports', () => {
        const idealLength = number('ideal length', 30);
        const flowSeparation = number('flow separation', 50);
        const nodesUnequalWithPorts: NodeSchema[] = kGraphFive.nodesUnequal.map((n) =>
            Object.assign(
                {
                    ports: {
                        e1: { location: 'east', order: 1 },
                        w1: { location: 'west', order: 1 },
                        w2: { location: 'west', order: 2 },
                        n1: { location: 'north', order: 1 },
                        s1: { location: 'south', order: 1 },
                        s2: { location: 'south', order: 2 },
                    },
                },
                n,
            ),
        ...
        const layout = makeLayout(nodesUnequalWithPorts, edgesTreeWithPorts, {
            idealLength,
            extraConstraints: function*(storage) {
                function constrain(uid: NodeId, vid: NodeId, direction: [number, number]) {
                    const u = storage.node(uid);
    ...
    .add('oriented edges', () => {
        const idealLength = number('ideal length', 30);
        const orientationAngle = number('orientation angle', 45, {
            range: true,
            min: 0,
            max: 360,
            step: 1,
        ...
        const layout = makeLayout(kGraphTwo.nodes, kGraphTwo.edges, {
            idealLength,
    ...
    .add('node distance and offset', () => {
        ...
        const layout = makeLayout(kGraphFive.nodesUnequal, kGraphFive.edgesTree, {
            ...
            extraConstraints: function*(storage) {
                yield [
                    ...constrainNodeOffset(storage.node('n0'), storage.node('n1'), '>=', 100, [
                        1,
                        1,
                    ]),
                    ...constrainNodeDistance(storage.node('n1'), storage.node('n2'), '>=', 100),
                    ...constrainNodeOffset(storage.node('n2'), storage.node('n3'), '>=', 100, [
                        -1,
                        1,
                    ]),
                    ...constrainNodeDistance(storage.node('n2'), storage.node('n4'), '>=', 100, {
                        axis: [1, 0],
                    }),
                ];
    ...
    .add('edge routing (gridified)', () => {
        ...
        const layout = makeLayout(kGraphGrid.nodes, kGraphGrid.edges, {
            idealLength,
            compactness: 1,
            padding,
    ...
    .add('edge routing (ports)', () => {
        ...
        const padding = number('padding', 10);
        const flowSeparation = number('flow separation', 50);
        const layout = makeLayout(kGraphPorts.nodes, kGraphPorts.edges, {
            idealLength,
            compactness: 1,
            padding,
            extraConstraints: function*(storage, step) {
                if (step > 10) {
                    for (let e of storage.edges()) {
                        if (
                            (storage as StructuredStorage).hasAncestorOrDescendant(
                                e.source.node,
                                e.target.node,
                            )
                        )
